...
function serializeMetrics(m: PerformanceMetrics): Record<string, unknown> {
  return {
    ...
    fundingCost: m.fundingCost.toString(),
...
function deserializeMetrics(raw: Record<string, unknown>): PerformanceMetrics {
  return {
    ...
    sortinoRatio: (raw.sortinoRatio as number) ?? 0,
    calmarRatio: (raw.calmarRatio as number) ?? 0,
    ...
    fundingCost: d((raw.fundingCost as string | undefined) ?? '0'),
